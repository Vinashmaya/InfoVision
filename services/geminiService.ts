...
// Use Gemini 3 models for high-quality research and visualization
const TEXT_MODEL = 'gemini-3-flash-preview';
// Pro model is required for accurate text rendering and high-quality typography
...
export const researchTopicForPrompt = async (
  ...
  const systemPrompt = `
    ...
    Your goal is to research the topic: "${topic}" and create a plan for a high-quality infographic.
    ...
    **IMPORTANT: Use the Google Search tool to find the most accurate, up-to-date information about this topic.**
    ...
    IMAGE_PROMPT:
    [Provide a highly detailed prompt for a high-end image generator. 
     CRITICAL: To ensure accurate text rendering, the prompt MUST explicitly list the exact text strings, labels, and titles to be shown in the image.
     Example: "An infographic about ${topic} with a main title '${topic}' and clear callouts for the following facts: [List Facts Here]. Use high-contrast, crisp, readable typography."
     Aesthetic parameters: ${styleInstr}. 
     Layout: Clean, professional, 16:9 aspect ratio.]
  `;
  ...
  const imagePrompt = promptMatch ? promptMatch[1].trim() : `Create a detailed infographic about ${topic} with legible labels. ${levelInstr} ${styleInstr}`;
...
export const generateInfographicImage = async (prompt: string): Promise<string> => {
  // Use Gemini 3 Pro with 2K resolution for superior text clarity
  const response = await getAi().models.generateContent({
    model: IMAGE_MODEL,
    contents: {
      parts: [{ text: prompt }]
  ...
  throw new Error("The model did not return an image. Please refine your prompt and try again.");
};
...
export const editInfographicImage = async (currentImageBase64: string, editInstruction: string): Promise<string> => {
  ...
  throw new Error("Image modification failed. Please try a different instruction.");
};

export const fixInfographicImage = async (currentImageBase64: string, correctionPrompt: string): Promise<string> => {
  return editInfographicImage(currentImageBase64, `Refine and improve this infographic based on: ${correctionPrompt}. Ensure visual clarity and accuracy of the text labels.`);
};
...
export const verifyInfographicAccuracy = async (
  ...
  return {
    isAccurate: true,
    critique: "Automated verification complete."
  };
};
